 ...
 * Update, July 19 2012: thanks to Taemin Kim, the code should
 *      now be compatible with more compilers, thanks to stricter
 *      traditional C code compliance.
 *
 *
 * Inputs (in order):
 ...
 *=================================================================*/
 ...
 /* NOTE:
  * The original code was modified to allow the use of the input parameter tol (optimality tolerance).
  * Leandro Prudente, July 2024 */


/* Jan 2018, tell the compiler to define this flag or manually uncomment this line, if you want ver 3.0 */
/* #define VER30 */
...
/* prototypes for the function and gradient evaluatioEsn routines */
...
void mexFunction( int nlhs, mxArray *plhs[], int nrhs, const mxArray*prhs[] )   { 
...
#ifdef VER30
    if (nrhs < 6 ) mexErrMsgTxt("ASA_CG ver 3.0 (Hager and Zhang)\nWrapper by Stephen Becker, 2018\nError: Needs at least 6 input arguments");
#else
    if (nrhs < 6 ) mexErrMsgTxt("ASA_CG ver 2.2 (Hager and Zhang)\nWrapper by Stephen Becker, 2012\nError: Needs at least 6 input arguments");
#endif
    ...
    grad_tol = mxGetScalar(prhs[N_tol]);
...
#ifdef VER30
    if (USE_FCN_GRAD==1)
        rValue = asa_cg(out, lo, hi, n, &Stat, &cgParm, &asaParm, grad_tol, myvalue, mygrad, myvalgrad, NULL, NULL) ; 
    else
        rValue = asa_cg(out, lo, hi, n, &Stat, &cgParm, &asaParm, grad_tol, myvalue, mygrad, NULL , NULL, NULL) ;
#else
    if (USE_FCN_GRAD==1)
        rValue = asa_cg(out, lo, hi, n, &Stat, &cgParm, &asaParm, grad_tol, myvalue, mygrad, myvalgrad, NULL) ; 
    else
        rValue = asa_cg(out, lo, hi, n, &Stat, &cgParm, &asaParm, grad_tol, myvalue, mygrad, NULL , NULL) ;
#endif
...
void parseOptions_CG( asacg_parm *cgParm, const mxArray *opts ){
    ...
    if (mxIsStruct( opts ) ) {
      ...
      /* the following were not in ver 2.2 */

            else if (strcmp(fnames[i],"LBFGS") == 0)
                cgParm->LBFGS = (int)value;
            else if (strcmp(fnames[i],"memory") == 0)
                cgParm->memory = (int)value;
            else if (strcmp(fnames[i],"SubCheck") == 0)
                cgParm->SubCheck = (int)value;
            else if (strcmp(fnames[i],"SubSkip") == 0)
                cgParm->SubSkip = (int)value;
            else if (strcmp(fnames[i],"eta0") == 0)
                cgParm->eta0 = (double)value;
            else if (strcmp(fnames[i],"eta1") == 0)
                cgParm->eta1 = (double)value;
            else if (strcmp(fnames[i],"eta2") == 0)
                cgParm->eta2 = (double)value;
            else if (strcmp(fnames[i],"nslow") == 0)
                cgParm->nslow = (int)value;
            else if (strcmp(fnames[i],"QuadSafe") == 0)
                cgParm->QuadSafe = (double)value;
            else if (strcmp(fnames[i],"UseCubic") == 0)
                cgParm->UseCubic = (int)value;
            else if (strcmp(fnames[i],"CubicCutOff") == 0)
                cgParm->CubicCutOff = (double)value;
            else if (strcmp(fnames[i],"SmallCost") == 0)
                cgParm->SmallCost = (double)value;
            else if (strcmp(fnames[i],"maxit") == 0)
                cgParm->maxit = (INT)value;
            else if (strcmp(fnames[i],"ntries") == 0)
                cgParm->ntries = (double)value;
            else if (strcmp(fnames[i],"ExpandSafe") == 0)
                cgParm->ExpandSafe = (double)value;
            else if (strcmp(fnames[i],"SecantAmp") == 0)
                cgParm->SecantAmp = (double)value;
            else if (strcmp(fnames[i],"RhoGrow") == 0)
                cgParm->RhoGrow = (double)value;
            else if (strcmp(fnames[i],"neps") == 0)
                cgParm->neps = (int)value;
            else if (strcmp(fnames[i],"nline") == 0)
                cgParm->nline = (int)value;
            ...
            if (strcmp(fnames[i], "PrintParms") == 0) 
                ...
                cgParm->step = (double)value;
            else if (strcmp(fnames[i], "maxit_fac") == 0)  /* not in ver 3.0 */
                cgParm->maxit_fac = (double)value;
            else if (strcmp(fnames[i], "totit_fac") == 0) /* not in ver 3.0 */
                cgParm->totit_fac = (double)value;
            else if (strcmp(fnames[i], "nexpand") == 0) /* not in ver 3.0 */
                cgParm->nexpand = (int)value;
            else if (strcmp(fnames[i], "nshrink") == 0) 
                cgParm->nshrink = (int)value;
            else if (strcmp(fnames[i], "nsecant") == 0) /* not in ver 3.0 */
...
void errorString( int errInt ) {
    ...
    switch (errInt ) {
        ...
        case 6: strcpy(errString,"excessive updating of eps in cg"); break;
        case 7: strcpy(errString,"Wolfe conditions never satisfied in cg"); break;
        case 8: strcpy(errString,"debugger is on and the function value increases in cg"); break;
        case 9: strcpy(errString,"no cost or gradient improvement in 2n + Parm->nslow iterations"); break;
        case 10: strcpy(errString,"out of memory in cg"); break;
        case 11: strcpy(errString,"function nan or +-INF and could not be repaired in cg"); break;
        case 12: strcpy(errString,"invalid choice for memory parameter for cg"); break;
        case 13: strcpy(errString,"out of memory"); break;
        case 14: strcpy(errString,"cbb iterations in all passes or in one pass exceeded their limit"); break;
        case 15: strcpy(errString,"line search failed in cbb iteration"); break;
        case 16: strcpy(errString,"search direction in cbb is not descent direction"); break;
        case 17: strcpy(errString,"function value became nan in cbb"); break;
