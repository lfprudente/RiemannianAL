...
#ifdef VER30
    if (nrhs < 5 ) mexErrMsgTxt("ASA_CG ver 3.0 (Hager and Zhang)\nWrapper by Stephen Becker, 2018\nError: Needs at least 5 input arguments");
#else
    if (nrhs < 5 ) mexErrMsgTxt("ASA_CG ver 2.2 (Hager and Zhang)\nWrapper by Stephen Becker, 2012\nError: Needs at least 5 input arguments");
#endif
...
#ifdef VER30
    if (USE_FCN_GRAD==1)
        rValue = asa_cg(out, lo, hi, n, &Stat, &cgParm, &asaParm, 1.e-8, myvalue, mygrad, myvalgrad, NULL, NULL) ; 
    else
        rValue = asa_cg(out, lo, hi, n, &Stat, &cgParm, &asaParm, 1.e-8, myvalue, mygrad, NULL , NULL, NULL) ;
#else
    if (USE_FCN_GRAD==1)
